...
class SarbacaneNewsWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct( 'sarbacane_newsletter', __( 'Mailify Newsletter', 'mailify' ), array(
			'description' => __( 'Have visitors fill out this form and the associated list will be updated accordingly', 'mailify' )
		) );
	}

	public function add_admin_menu() {
		add_submenu_page( 'sarbacane', __( 'Widget', 'mailify' ), __( 'Widget', 'mailify' ), 'administrator', 'wp_news_widget', array(
			$this,
			'display_settings'
		) );
	}
	...
	public function sarbacane_save_widget() {
		$valid = false;
		if ( isset ( $_POST ['sarbacane_form_value'] ) && isset ( $_POST ['email'] ) ) {
			...
			if ($sarbacane_form_value == 'sarbacane_desktop_widget' ) {
				...
				if ( is_email( $user_registering->email ) ) {
					...
					if ( $version !== false ) {
						global $wpdb;
						$sql = "INSERT INTO `{$wpdb->prefix}sd_subscribers` (`email`, `columns`, `registration_date`) VALUES (%s, %s, %s);";
						$result = $wpdb->query( $wpdb->prepare( $sql, $user_registering->email, json_encode( $columns ), $user_registering->registration_date ) );
					}
		...
		$this->display_registration_message($valid);
	}

	public function display_registration_message($valid) {
		if ( $valid ) {
			$message = json_encode( array( 'message' => get_option( 'sarbacane_news_registration_message', __( 'Congrats! You signed up for our newsletter.', 'mailify' ) ) ) );
		} else {
			$message = json_encode( array( 'message' => __( 'Email isn\'t valid.', 'mailify' ) ) );
		}
		echo '
	...
	public function widget( $args, $instance ) {
		$title = get_option( 'sarbacane_news_title', __( 'Newsletter', 'mailify' ) );
		...
		if ( ! is_array( $fields ) || count( $fields ) == 0 ) {
			...
			update_option( 'sarbacane_news_fields', array( $default_email ), false );
			$fields = get_option( 'sarbacane_news_fields' );
		}
		$registration_button = get_option( 'sarbacane_news_registration_button', __( 'Inscription', 'mailify' ) );
		$registration_mandatory_fields = get_option( 'sarbacane_news_registration_mandatory_fields', __( 'Fields marked with * are mandatory', 'mailify' ) );
		$list_type = 'N';
		$rand = mt_rand( 0, 1000000 );
		wp_enqueue_script( 'sarbacane-widget.js', plugins_url( 'js/sarbacane-widget.js', __FILE__ ), array( 'jquery' ), '1.4.7' );
		wp_enqueue_style( 'sarbacane_widget.css', plugins_url( 'css/sarbacane_widget.css', __FILE__ ), array(), '1.4.7' );
	...
	public function form( $instance ) {
		_e( 'Setup this widget by clicking the Mailify widget menu', 'mailify' );
	}
	...
	public function display_settings() {
		...
		wp_enqueue_style( 'sarbacane_global.css', plugins_url( 'css/sarbacane_global.css', __FILE__ ), array( 'wp-admin' ), '1.4.7' );
		wp_enqueue_style( 'sarbacane_widget_admin_panel.css', plugins_url( 'css/sarbacane_widget_admin_panel.css', __FILE__ ), array( 'wp-admin' ), '1.4.7' );
		wp_enqueue_script( 'sarbacane-widget-adminpanel.js', plugins_url( 'js/sarbacane-widget-adminpanel.js', __FILE__ ), array( 'jquery', 'underscore' ), '1.4.7' );
		$nonce_ok = false;
		...
		if ( $nonce_ok && isset ( $_POST ['sarbacane_save_configuration'] ) ) {
			if ( isset ( $_POST ['sarbacane_widget_title'] ) && isset ( $_POST ['sarbacane_widget_description'] ) && isset ( $_POST ['sarbacane_widget_registration_button'] ) ) {
				if ( isset ( $_POST ['sarbacane_widget_registration_message'] ) && isset ( $_POST ['sarbacane_field_number'] ) && isset ( $_POST ['sarbacane_widget_registration_mandatory_fields'] ) ) {
		...
		$title = get_option( 'sarbacane_news_title', __( 'Newsletter', 'mailify' ) );
		$description = get_option( 'sarbacane_news_description', '' );
		$registration_message = get_option( 'sarbacane_news_registration_message', __( 'Congrats! You signed up for our newsletter.', 'mailify' ) );
		$registration_button = get_option( 'sarbacane_news_registration_button', __( 'Inscription', 'mailify' ) );
		$registration_mandatory_fields = get_option( 'sarbacane_news_registration_mandatory_fields', __( 'Fields marked with * are mandatory', 'mailify' ) );
	...
	public function save_parameters( $sanitized_post ) {
		...
		update_option( 'sarbacane_news_title', $title, false );
		update_option( 'sarbacane_news_description', $description, false );
		update_option( 'sarbacane_news_fields', $new_fields, false );
		update_option( 'sarbacane_news_registration_message', $registration_message, false );
		update_option( 'sarbacane_news_registration_button', $registration_button, false );
		update_option( 'sarbacane_news_registration_mandatory_fields', $registration_mandatory_fields, false );
	}
