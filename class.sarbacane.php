...
class Sarbacane {
	...
	public static function activation() {
		update_option( 'sarbacane_version', '1.4.7', false );
		update_option( 'sarbacane_sd_token', '', false );
		update_option( 'sarbacane_sd_id_list', array(), false );
		update_option( 'sarbacane_news_registration_button', __( 'Inscription', 'mailify' ), false );
							...
							foreach ( $subscribers as $key => $one_subscriber ) {
								if ( isset( $one_subscriber->email ) ) {
									...
									if ( $key % 1000 == 0 || $key + 1 == $nb_subscribers ) {
										$sql = "INSERT INTO `{$wpdb->prefix}sd_subscribers` (`email`, `columns`, `registration_date`) VALUES " . implode( ',', $insert_request ) . ";";
	...
	public function add_admin_menu() {
		add_submenu_page( 'sarbacane', __( 'Connection', 'mailify' ), __( 'Connection', 'mailify' ), 'administrator', 'wp_interconnection', array(
			$this,
			'display_settings'
		) );
	}
	...
	public function sarbacane_load_locales() {
		load_plugin_textdomain( 'mailify', false, dirname( plugin_basename( __FILE__ ) ) . '/locales' );
	}

	/**
	 * Display message when key generated
	 */
	private function sarbacane_params_saved() {
		echo '<div class="updated">' . __( 'A new key has been generated', 'mailify' ) . '</div>';
	}
	 ...
	 *            If empty, get lists vide else get email for the list
	 *
	 * @throws WP_Error
	 */
	...
	/**
	 * Check new users since last call
	 *
	 * @param string $last_call_date
	 *
	 * @return boolean
	 */
	...
	/**
	 * Check new newsletter subcribers since last call
	 *
	 * @param string $last_call_date
	 *
	 * @return boolean
	...
	private function get_users_list( $sd_id ) {
		if ( ! get_option( 'sarbacane_users_list' ) ) {
			echo $this->generate_csv( array(), array(), 'U' );
			return;
		}
	 ...
	 * @return CSV
	 */
	private function generate_csv( $new_or_updated_users, $deleted_users = array(), $list_type ) {
		if ( "N" == $list_type ) {
	...
	private function sarbacanedesktop_get_all_users( $last_call_date ) {
		global $wpdb;
		$sql = "SELECT `wu`.`id` AS `user_id`, `wu`.`user_login` AS `user_login`, `wu`.`user_email` AS `email`, `wmf`.`meta_value` AS `firstname`, `wml`.`meta_value` AS `lastname`, `wum`.`meta_value` AS `user_role`
		FROM `{$wpdb->prefix}users` AS `wu` LEFT JOIN `{$wpdb->prefix}usermeta` AS `wmf` ON `wu`.`id` = `wmf`.`user_id` AND `wmf`.`meta_key` = 'first_name'
		LEFT JOIN `{$wpdb->prefix}usermeta` AS `wml` ON `wu`.`id` = `wml`.`user_id` AND `wml`.`meta_key` = 'last_name'
		LEFT JOIN `{$wpdb->prefix}usermeta` AS `wum` ON `wu`.`id` = `wum`.`user_id` AND `wum`.`meta_key` = '{$wpdb->prefix}capabilities'";
		if ( $last_call_date == '' ) {
			$all_users = $wpdb->get_results( $sql );
		} else {
			$sql = $sql . " LEFT JOIN `{$wpdb->prefix}sd_updates` AS `wsu` ON `wsu`.`user_id` = `wu`.`id` AND `wsu`.`action` = 'S' WHERE `wu`.`user_registered` >= %s OR `wsu`.`time` >= %s";
	...
	private function sarbacanedesktop_get_all_deleted_users( $last_call_date ) {
		global $wpdb;
		$sql = "SELECT `user_email` FROM `{$wpdb->prefix}sd_updates` AS `wsu` WHERE `wsu`.`action` = 'U' AND `wsu`.`time` >= %s";
	...
	private function get_subscribers( $last_call_date ) {
		global $wpdb;
		if ( $last_call_date != '' ) {
			$sql = "SELECT * FROM `{$wpdb->prefix}sd_subscribers` WHERE `registration_date` > %s ORDER BY `registration_date` ASC";
			$all_subscribers = $wpdb->get_results( $wpdb->prepare( $sql, $last_call_date ) );
		} else {
			$sql = "SELECT * FROM `{$wpdb->prefix}sd_subscribers` ORDER BY `registration_date` ASC";
	...
	private function clear_update_history() {
		global $wpdb;
		$sql = "DELETE FROM `{$wpdb->prefix}sd_updates` WHERE `time` <= (
			SELECT MIN(`option_value`) FROM `{$wpdb->prefix}options` WHERE `option_name` LIKE 'sarbacane_%_call_%'
		)";
		$wpdb->query( $sql );
	}
	...
	public function display_settings() {
		...
		wp_enqueue_style ( 'sarbacane_admin_panel.css', plugins_url ( 'css/sarbacane_admin_panel.css', __FILE__ ), array(), '1.4.7' );
	...
	public function sarbacane_process_request( $wp ) {
		...
		if ( isset ( $wp->query_vars ['sarbacane_form_token'] ) && $wp->query_vars ['sarbacane_form_token'] != '' && wp_verify_nonce( $wp->query_vars['sarbacane_form_token'], 'newsletter_registration' ) ) {
			$save_widget = true;
		}
		...
		if ( $sd_token != $sd_token_saved || $sd_token_saved == '' || $sd_token_saved == false || strlen( $sd_token_saved ) < 20 || $is_failed ) {
			if ( ! $is_failed ) {
			...
			if ( isset ( $type ) ) {
				if ( $type == 'posts' ) {
					...
					if ( isset ( $id ) ) {
						$content->get_article_rss( $id );
					} else {
						$content->get_articles_rss( $limit );
					}
				...
				if ( $action == 'delete' ) {
					...
					$this->sarbacane_get_list( $sd_id, $sd_list_id );
				}
			}
		}
		exit;
